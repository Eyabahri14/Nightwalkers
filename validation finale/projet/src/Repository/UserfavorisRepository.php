...
class UserfavorisRepository extends ServiceEntityRepository
{
    ...
    public function GetAllarticle($id): array
    {
        $conn = $this->getEntityManager()->getConnection();

        $sql = '
SELECT * FROM `article` inner JOIN userfavoris_article on article.id = userfavoris_article.article_id WHERE userfavoris_article.userfavoris_id = '. $id
            ;
